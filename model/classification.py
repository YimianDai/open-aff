...
from .block import ResBlockV2ASKC, BottleneckV1bASKC, ResNeXtBlockASKC
...
class ResNextASKC(HybridBlock):
    ...
    def __init__(self, askc_type, layers, cardinality, bottleneck_width,
        ...
        super(ResNextASKC, self).__init__(**kwargs)
        ...
        with self.name_scope():
            ...
            for i, num_layer in enumerate(layers):
                stride = 1 if i == 0 else 2
                self.features.add(self._make_layer(askc_type, channels, num_layer, stride, last_gamma,
                                                   use_se, False if i == 0 else avg_down, i + 1,
                                                   norm_layer=norm_layer, norm_kwargs=norm_kwargs))
                channels *= 2
            self.features.add(nn.GlobalAvgPool2D())
    ...
    def _make_layer(self, askc_type, channels, num_layers, stride, last_gamma, use_se, avg_down,
                    stage_index, norm_layer=BatchNorm, norm_kwargs=None):
        layer = nn.HybridSequential(prefix='stage%d_' % stage_index)
        if stage_index <= 2:
            askc_type = 'DirectAdd'
        with layer.name_scope():
            layer.add(ResNeXtBlockASKC(askc_type, channels, self.cardinality, self.bottleneck_width,
...
class ResNet110V2ASKC(HybridBlock):
    def __init__(self, in_askc_type, layers, channels, classes, start_layer,
                 norm_layer=BatchNorm, norm_kwargs=None, **kwargs):
        super(ResNet110V2ASKC, self).__init__(**kwargs)
        assert len(layers) == len(channels) - 1
        with self.name_scope():
            self.features = nn.HybridSequential(prefix='')
            self.features.add(norm_layer(scale=False, center=False,
            ...
            for i, num_layer in enumerate(layers):
                if i + 1 >= start_layer:
                    askc_type = in_askc_type
                else:
                    askc_type = "DirectAdd"

                stride = 1 if i == 0 else 2
                self.features.add(self._make_layer(
                    askc_type, layers=num_layer, channels=channels[i+1], in_channels=in_channels,
                    stride=stride, stage_index=i+1, norm_layer=norm_layer, norm_kwargs=norm_kwargs))
                in_channels = channels[i+1]
    ...
    def _make_layer(self, askc_type, layers, channels, in_channels, stride, stage_index,
...
class CIFARResNextASKC(HybridBlock):
    ...
    norm_kwargs : dict
        ...
        for :class:`mxnet.gluon.contrib.nn.SyncBatchNorm`.
    """
    def __init__(self, layers, cardinality, bottleneck_width, classes=10, use_se=False,
                 norm_layer=BatchNorm, norm_kwargs=None, **kwargs):
        super(CIFARResNextASKC, self).__init__(**kwargs)
        self.cardinality = cardinality
        self.bottleneck_width = bottleneck_width
        channels = 64
        ...
        with self.name_scope():
            ...
            for i, num_layer in enumerate(layers):
                stride = 1 if i == 0 else 2
                self.features.add(self._make_layer(use_se, channels, num_layer, stride, i+1,
                                                   norm_layer=norm_layer, norm_kwargs=norm_kwargs))
                channels *= 2
            self.features.add(nn.GlobalAvgPool2D())
    ...
    def _make_layer(self, use_se, channels, num_layer, stride, stage_index,
                    norm_layer=BatchNorm, norm_kwargs=None):
        layer = nn.HybridSequential(prefix='stage%d_'%stage_index)
        with layer.name_scope():
            # layer.add(ResNeXtBlockASKC('ResGlobLocaforGlobLocaCha', channels, self.cardinality, self.bottleneck_width,
            ...
            layer.add(ResNeXtBlockASKC('ASKCFuse', channels, self.cardinality, self.bottleneck_width,
                                 stride, True, use_se=use_se, prefix='',
                                 norm_layer=norm_layer, norm_kwargs=norm_kwargs))
            for _ in range(num_layer-1):
                layer.add(ResNeXtBlockASKC('ASKCFuse', channels, self.cardinality, self.bottleneck_width,
